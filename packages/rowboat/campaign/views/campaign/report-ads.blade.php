...
<div class="roles-wrap wrap-branch" data-ng-controller="campaignDetailCtrl" >
	<div class="top-content">
	    <h3 class="c-m"><a style="color: #fff" href="{{ URL::to('campaign')}}">Campaigns</a> > <a style="color: #fff" href="{{ URL::to('campaign/report-group')}}/{{$campaignId}}">{{$campaignName}}</a> > 
	    {{$groupName}}</h3>
	</div>
		...
		<form name="formSearchByDate" novalidate>
			<div class="tile-title">
	            ...
	            <div class="search-day col-sm-12 col-md-2 col-lg-4 form-group">
	            	<button ng-disable="isLoading" style="padding-left:24px;padding-right:24px" class="btn btn-default" ng-click="getSearchModalAds('search-report-ads', formSearchByDate.$invalid)">Search</button>
            ...
            <table ng-show="!isLoading" class="table table-bordered" ng-table="tableParams">
			    <tbody>
				    <tr ng-repeat="campaign in $data">
					    ...
					    <td sortable="'name'" data-title="'Name'" class="text-center">@{{campaign['name']}}</td>
		                ...
		                <td sortable="'Conversion'" data-title="'Conversion'" class="text-center">@{{campaign['Conversion']}}</td>
		                <td sortable="'Avgposition'" data-title="'Avgposition'" class="text-center">@{{campaign['Avgposition']}}</td>
		            </tr>
