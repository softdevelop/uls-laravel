        ...
        link: function($scope, element, attrs, ngModel) {
            ...
            $scope.$watch('searchName', function(newVal, oldVal) {
                ...
                if (angular.isDefined(newVal) && newVal != '') {
                    ...
                    // Close all ul, icon plus to minus
                    $timeout(function(){
                        // Format tag
            ...
            $scope.getFirstLevelOfTagSearched = function (tagValidWithSearch) {
                ...
                angular.forEach(tagValidWithSearch, function(value, key) {
                    ...
                    if(value['parent_id'] == '0') {
                        ...
                        //     $('.parrent-select-' + value1).removeClass("off");
                        //     // $('.parrent-select-' + value1).parent().find('i').removeClass('ti-plus').addClass('ti-minus');
                        // })
                    }
                    angular.forEach(value, function(value1, key1) {
                        console.log(value1, 'dsa');
                        // $('.parrent-select-' + value1).removeClass("off");
                        // $('.parrent-select-' + value1).parent().find('i').removeClass('ti-plus').addClass('ti-minus');
                    })
            ...
            $scope.toggleAllChildTag = function () {
                ...
                if (angular.isDefined($scope.currentPage.tags) || $scope.currentPage.tags != []) {

                    $timeout(function(){
                        // Each tag of page
                        angular.forEach($scope.currentPage.tags, function(value, key) {
                    ...
                    $scope.expandAllChildTagWithTagIds();
                }
            }
             ...
             * @return {Void}
             */
            $scope.expandAllChildTagWithTagIds = function() {
                $timeout(function(){
                    // Earch parentTagIds
            ...
            /**
             * Get all parrent id of current tag activated
             *
             * @author Thanh Tuan <tuan@httsolution.com>
             *
             * @param  {String} tagId Tag id of page
             *
             * @return {Void}
             */
            $scope.getAllParentIfOfCurrentTagActivated = function (tagId) {
                // Earch tag
                for (var key in $scope.allTags) {
                    // If tagId of page = tag[key]
                    if ($scope.allTags[key]._id == tagId) {
                        // If different tag has parent_id = 0
                        if ($scope.allTags[key].parent_id != '0') {
                            // Push parent tag id to array
                            parentTagIds.push($scope.allTags[key].parent_id);
                            // Call function to get all parent id of curent tag activated
                            $scope.getAllParentIfOfCurrentTagActivated($scope.allTags[key].parent_id);
                        }
                    }
                }
            }
