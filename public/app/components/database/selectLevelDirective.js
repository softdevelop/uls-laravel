...
selectLevel.directive("selectLevel", ['$timeout', '$filter', function($timeout, $filter) {
    return {
        ...
        replace: true,
        templateUrl: baseUrl + '/app/components/database/views/view.html?v=' + new Date().getTime(),
        link: function($scope, element, attrs, ngModel) {
            ...
            $scope.selectedItem = function(item, $event) {
                ...
                if ((angular.isDefined(item.parentIds) || angular.isDefined(item.parentId)) && item.name !='') {
                    $scope.text = item.name;
                } else {
                    // $scope.text = 'Select Folder';
                }
                ...
                // console.log(itemIdNeedActived,' test');
