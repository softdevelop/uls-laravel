...
(function(angular){
    ...
    .directive('ngJcrop', ['ngJcropConfig', function(ngJcropConfig){
        ...
        return {
            restrict: 'A',
            scope: { ngJcrop: '=', thumbnail: '=', selection: '=', ngJcropConfigName: '@' , config: '=', ratio: '='},
    ...
    function($scope, $element, ngJcropConfig){
        ...
        ngJcropConfig.jcrop = $scope.config;
        console.log($scope.config, 'bbb');
        ...
        $scope.updateCurrentSizes = function(img){
            console.log(widthShrinkRatio, 'widthShrinkRatio');
        ...
        $scope.updateCoords = function(coords){
            $scope.$apply(function(){
                $scope.setSelection(coords);
                // $scope.result = coords;
            });
        ...
        $scope.onMainImageLoad = function(){
            ...
            console.log('toan', $scope.config);
            // $scope.updateCoords($scope.coords);
        ...
        $scope.$watch('ratio', function(newValue, oldValue, scope){
            if(!$scope.jcrop) return;
            var config = angular.extend({
                onChange: $scope.updateCoords,
                onSelect: $scope.updateCoords
            }, $scope.config);
            ...
            console.log(config, 'ituirtiru');
            $scope.jcrop = jQuery.Jcrop($scope.mainImg[0], config);
            // $scope.jcrop.setOptions({aspectRatio: 16/9});
        })
