...
<div ng-if="field.form">
    ...
    <div class="wrap-box-title" ng-if="field.multiple">
        ...
        <a id="addFieldType_@{{$index}}" class="pointer btn btn-xs btn-upload pull-right" ng-click="addNewField(field, 'current')"  ng-if="field.max_field > 1 || !field.max_field">
            <i class="fa fa-plus-square"></i>  Add
        </a>
        <div id="parentBlock_@{{field.variable}}" class="collapse">
            <div>
                <div class="panel-group" role="tablist" aria-multiselectable="true" ng-repeat="(key, field_el) in multiFieldFollowVariable['_' + currentBlockInject + '_' + field.variable]" class="sectionsid">
                    ...
                    <div class="panel panel-default have-border">
                        <div class="panel-heading have-bg fix-height" role="tab">
                            <h4 class="panel-title relative">
                                <span class="position-close" ng-click="removeCurrentField(field, field_el.key_field, 'current')" ng-show="field_el.key_field != 0 || field.min_field == 0">
                                    <i class="ti-close"></i>
                                </span>
...
<div ng-if="!field.form">
    <div ng-if="listOutTypeMap[field.type] != 'select' && listOutTypeMap[field.type] != 'textarea'">
        @include('cms-content.partial.input-template-current')
    </div>
    <div id="field_@{{field._id.$id}}" ng-if="listOutTypeMap[field.type] == 'select'">
        @include('cms-content.partial.select-template-current')
    </div>
    <div id="field_@{{field._id.$id}}" ng-if="listOutTypeMap[field.type] == 'textarea'">
        @include('cms-content.partial.text-area-template-current')
    </div>
    <div class="clearfix"></div>
</div>
