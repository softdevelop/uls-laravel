...
<div class="modal-body">
    <form role="form" name="createFolderForm" novalidate>
        ...
        <!-- Input Name-->
        <div class="form-group" ng-init="parentFolders = {{json_encode($result)}}; selectedItem = {{json_encode($selectedItem)}}" ng-class="[submitted && (createFolderForm.name.$invalid || nameExists)]">
        ...
        <div class="form-group">
            <label class="label-form" for="name">
                {{trans('cms_asset/add-folder.folder_parent_label')}}:<span class="text-require"> *</span>
            </label>

            <div class="wrap-form">

                <select-level items="parentFolders" show-icon="true" text="{{trans('cms_asset/add-folder.folder_parent_select')}}" text-filter="Filter folder" ng-model="assetManager.parent_id" selected-item="selectedItem"></select-level>
                <div class="pull-left">
                    <small class="help-inline" ng-show="submitted && !assetManager.parent_id">{{trans('cms_asset/add-folder.folder_parent_required')}}</small>
                </div>
            </div>
            <div class="clearfix"></div>
        </div>
